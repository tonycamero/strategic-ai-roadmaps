-- =============================================
-- ROBERTA HAYES DEMO FIRM SEED DATA (V2)
...
-- 1. CREATE OWNER USER (Roberta Hayes)
INSERT INTO users (
  email,
  password_hash,
  name,
...
FROM (SELECT gen_random_uuid() AS id) tmp
RETURNING id, email;

-- Store the owner_id for subsequent use (we'll reference via subquery)
...
-- Director of Operations - Michael Chen
INSERT INTO users (
  email,
  password_hash,
  name,
...
-- Senior Sales Agent - Sarah Mitchell
INSERT INTO users (
  email,
  password_hash,
  name,
...
-- Marketing & Client Coordination - Jasmine Rivera
INSERT INTO users (
  email,
  password_hash,
  name,
...
-- Transaction Coordinator - David Kim
INSERT INTO users (
  email,
  password_hash,
  name,
...
-- Intake #1: Lead Management & Follow-up System (by Roberta)
INSERT INTO intakes (
  user_id,
  role,
  answers,
...
-- Intake #2: Client Onboarding & Document Collection (by Michael Chen)
INSERT INTO intakes (
  user_id,
  role,
  answers,
...
-- Intake #3: Agent Performance & Activity Tracking (by Roberta)
INSERT INTO intakes (
  user_id,
  role,
  answers,
...
-- =============================================
-- Query to verify:
-- SELECT 'Tenant' as type, name FROM tenants WHERE owner_id = (SELECT id FROM users WHERE email = 'roberta@hayesrealestate.com')
-- UNION ALL
-- SELECT 'User', name FROM users WHERE email LIKE '%hayesrealestate.com'
-- UNION ALL
-- SELECT 'Intake', answers->>'workflow_name' FROM intakes WHERE owner_id = (SELECT id FROM users WHERE email = 'roberta@hayesrealestate.com');
-- =============================================
