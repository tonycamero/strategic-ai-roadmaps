...
import { db } from '../db';
import { users, intakes, tenants, roadmaps, auditEvents, tenantDocuments, discoveryCallNotes, roadmapSections, ticketPacks, ticketInstances, tenantMetricsDaily, webinarRegistrations, onboardingStates, implementationSnapshots, roadmapOutcomes, agentConfigs, agentThreads, webinarSettings, executiveBriefs, intakeVectors, sopTickets } from '../db/schema';
...
import { ingestDiagnostic, ArtifactNotFoundError } from '../services/diagnosticIngestion.service';
...
export async function getFirmWorkflowStatus(req: AuthRequest, res: Response) {
  try {
    ...
    const [discoveryNote] = await db
      ...
      .from(discoveryCallNotes)
      .where(eq(discoveryCallNotes.tenantId, tenantId))
      .limit(1); // Added limit(1) for consistency

    // 4. Query roadmap sections
    ...
    const roadmapComplete = roadmapDocs.length > 0; // Simple check for existence
    ...
    return res.json({
      ...
      sop01: {
        ...
        outputsGenerated: sop01Docs.length,
      },
      discovery: {
...
export async function generateSop01ForFirm(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!brief || brief.status !== 'APPROVED') {
      return res.status(403).json({
        error: 'Gated Action',
        details: 'Executive Brief must be APPROVED before executing diagnostic synthesis.'
      });
    }

    // GATING: Intake Window must be CLOSED (Waterfall Step 1 Complete)
    const [intakeVector] = await db
      .select({ status: intakeVectors.status })
      .from(intakeVectors)
      .where(and(
        eq(intakeVectors.tenantId, tenantId),
        eq(intakeVectors.role, 'owner') // Checking owner/main intake vector state
      ))
      .limit(1);

    // Note: In v2, we might check a global tenant flag, but checking the owner vector or a specific 'intake_window_state' is proxy.
    // For now, Brief Approval implies strict freeze, so the Brief check above is the primary gate.
    // We will trust the Brief check as authoritative for Step 2 entry.

    // Build normalized intake context
    const normalized = await buildNormalizedIntakeContext(tenantId);

    // Generate SOP-01 outputs via AI (Canonical Contract)
    const outputs = await generateSop01Outputs(normalized);

    // Persist SOP-01 Artifacts (Hard Requirement - Atomic DB writes)
    try {
      await persistSop01OutputsForTenant(tenantId, outputs);
    } catch (error: any) {
      console.error('[SOP-01] Persistence Failed:', error);
      return res.status(500).json({
        error: 'SOP01_PERSIST_FAILED',
    ...
    // Ingest!
    // This will:
    // 1. Map canonical artifacts to prompt
    // 2. Extract tickets via Ticket Architect
    // 3. Save to sop_tickets table (status='proposed', approved=false)
    let ingestionResult;
    try {
      ...
      if (error instanceof ArtifactNotFoundError) {
        return res.status(428).json({
          error: 'SOP01_ARTIFACTS_NOT_FOUND',
          details: 'Core diagnostic artifacts are missing or incomplete.',
          prerequisites: error.prerequisites
        });
      }
      console.error('[SOP-01] Ingestion Error:', error);
      throw error; // Re-throw generic 500s
    }
    ...
    await db.update(onboardingStates)
      .set({
        ...
        currentStep: 'roadmap_review' // Move to next step
      })
      .where(eq(onboardingStates.tenantId, tenantId));
    ...
    console.error('Error generating SOP-01:', error);
    next(error);
  }
}

/**
 * NEW: Assemble Final Roadmap from Approved Tickets
 * STRICT WATERFALL STEP 5
 */
export async function assembleRoadmapForFirm(req: AuthRequest, res: Response, next: NextFunction) {
  const { tenantId } = req.params;

  try {
    // 1. Verify Executive Brief is APPROVED
    const [brief] = await db
    ...
    if (!brief || brief.status !== 'APPROVED') {
      return res.status(428).json({
        error: 'ROADMAP_NOT_READY',
        message: 'Executive brief is not approved.',
        prerequisites: {
          hasApprovedBrief: false,
          hasDiagnostic: false, // Unknown at this stage
          approvedTicketCount: 0
        }
      });
    }

    // 2. Identify Active Diagnostic (Pinning)
    // We strictly use the most recently generated diagnostic to prevent phantom tickets from
    // previous cycles bleeding into the current roadmap.
    // In V2 this might come from `input` body, but for now we infer LATEST.
    const [latestDiagnostic] = await db
      .select({ id: sopTickets.diagnosticId })
      .from(sopTickets) // This is safe because tickets are created WITH diagnosticId
      .where(eq(sopTickets.tenantId, tenantId))
      .orderBy(desc(sopTickets.createdAt))
      .limit(1);

    const targetDiagnosticId = latestDiagnostic?.id;

    if (!targetDiagnosticId) {
      return res.status(428).json({
        error: 'ROADMAP_NOT_READY',
    ...
    // 3. Fetch APPROVED Tickets (Pinned)
    const approvedTickets = await db
      ...
      .where(and(
        ...
        eq(sopTickets.diagnosticId, targetDiagnosticId) // PINNED
      ))
      .orderBy(asc(sopTickets.createdAt));

    if (approvedTickets.length === 0) {
      console.warn(`[SOP-01] Roadmap Assembly blocked: 0 approved tickets for tenant ${tenantId} (Diag: ${targetDiagnosticId})`);
      return res.status(428).json({
        error: 'ROADMAP_NOT_READY',
        message: 'No approved tickets found for the active diagnostic.',
        prerequisites: {
          hasApprovedBrief: true,
          hasDiagnostic: true,
          approvedTicketCount: 0
    ...
    const packs: any[] = [];
    const groupedBySprint: Record<string, typeof approvedTickets> = {};

    for (const t of approvedTickets) {
      const key = `Phase ${t.sprint || 1}`;
      if (!groupedBySprint[key]) groupedBySprint[key] = [];
      groupedBySprint[key].push(t);
    }

    Object.entries(groupedBySprint).forEach(([phaseName, tickets], idx) => {
      packs.push({
        id: `pack-${idx + 1}`,
        title: phaseName,
    ...
    console.log(`[SOP-01] Assembled Roadmap with ${approvedTickets.length} tickets into ${packs.length} packs.`);

    return res.status(200).json({
      success: true,
      roadmap: {
        tenantId,
        assembledFromDiagnosticId: targetDiagnosticId,
