...
// Use relative path from scripts/ back to src/
...
async function main() {
    ...
    // Double check if *any* diagnostic exists for this tenant
    ...
    const allDiags = await db
        ...
        .orderBy(diagnostics.createdAt); // removed desc() to fix import dependency, just standard orderBy

    console.log('All diagnostics for tenant:', allDiags.length);
    allDiags.forEach(d => console.log(`- ID: ${d.id}, Status: ${d.status}`));
