            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="flex gap-2">
                        {status === 'DRAFT' && brief && (
                            <button
                                onClick={() => handleStatusTransition('READY_FOR_EXEC')}
                                disabled={isSaving}
                        ...
                        {status === 'READY_FOR_EXEC' && (
                            <>
                                <button
                                    onClick={() => handleStatusTransition('WAIVED')}
                                    disabled={isSaving}
...
export const ExecutiveBriefSurface: React.FC<ExecutiveBriefSurfaceProps> = ({ onStatusChange }) => {
    ...
    const handleStatusTransition = async (newStatus: ExecutiveBriefStatus, reason?: string) => {
        setIsSaving(true);
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`/api/superadmin/firms/${tenantId}/exec-brief/status`, {
