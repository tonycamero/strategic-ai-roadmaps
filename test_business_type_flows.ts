...
async function register(email: string, name: string, company: string): Promise<RegisterResponse> {
...
async function updateBusinessType(token: string, businessType: 'default' | 'chamber'): Promise<void> {
  ...
  console.log(`  ✓ Updated to ${businessType}:`, data.tenant);
}
